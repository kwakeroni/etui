...
if $FROM_SOURCE; then
  ...
  echo Starting Etui
  "$JAVA_HOME/bin/java" --source 22 -splash:src/main/resources/splash.png -cp @.classpath-src src/main/java/Etui.java "$@"
else
  ...
  echo Starting Etui
  "$JAVA_HOME/bin/java" -splash:src/main/resources/splash.png -cp @.classpath Etui "$@"
fi
